...
public class Evaluator {
	...
	private boolean legal(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (operators.contains(currChar) && countEquals == 0 && input.contains("=")) {
	...
	//checks for legal arguments, if it's good then it assigns the right side of = to the left side, if it's just an expression it solves it
	...
	private String eval(String infix) {
		InfixToPostfix expressions = new InfixToPostfix(infix);
